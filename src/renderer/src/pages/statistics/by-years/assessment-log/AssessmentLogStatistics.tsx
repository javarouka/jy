import useAssessmentLogStatistics from '@renderer/pages/statistics/by-years/assessment-log/hook/useAssessmentLogStatistics'
import SubTitle from '@renderer/component/basic/SubTitle'
import { convertMinuteToReader } from '@renderer/helpers/Times'

export default function AssessmentLogStatistics() {
  const { statisticsData, isLoading, isError } = useAssessmentLogStatistics()
  ...
  return (
    <div className="w-full">
      <SubTitle className="mb-4">심리평가 통계</SubTitle>
          ...
          <div className="space-y-4">
            {statisticsData.map((data) => (
              <div key={data.name} className="border rounded-lg p-4 border-gray-200 dark:border-gray-700">
                ...
                {/* Display statistics for all researchType values */}
                <div className="mt-6">
                  <h5 className="text-sm font-medium mb-3">검사 종류별 통계</h5>
                  <div className="space-y-4">
                    {data.typeStats.map((typeStat) => (
                      <div
                        key={typeStat.type}
                        className={`p-3 rounded-lg ${
                          typeStat.isComprehensive
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          <h6 className={`text-sm font-medium ${
                            typeStat.isComprehensive ? 'text-blue-700 dark:text-blue-300' : ''
                          }`}>
                            {typeStat.type}
                            {typeStat.isComprehensive && ' ★'}
                          </h6>
