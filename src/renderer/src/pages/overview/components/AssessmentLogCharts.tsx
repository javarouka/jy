...
interface AssessmentLogChartsProps {
  trainingYear?: TypeTrainingYear;
}

const AssessmentLogCharts = ({ trainingYear }: AssessmentLogChartsProps) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const { data: assessmentLogs, isLoading, error } = useAssessmentLogs(trainingYear);
  ...
  // Process age data
  const ageData = assessmentLogs.reduce((acc, log) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="text-lg font-semibold mb-4">월별 심리평가 건수 및 인정시간</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <h3 className="text-lg font-semibold mb-4">검사 유형 분포</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                {...{ activeIndex } as any}
...
export default AssessmentLogCharts;
