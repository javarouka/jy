...
interface OtherActivityLogChartsProps {
  trainingYear?: TypeTrainingYear;
}

const OtherActivityLogCharts = ({ trainingYear }: OtherActivityLogChartsProps) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const { data: activityLogs, isLoading, error } = useOtherActivityLogs(trainingYear);
  ...
  const monthlyData = groupByMonth(activityLogs, 'startDate');

  // Process data for yearly bar charts
  const yearlyData = groupByYear(activityLogs, 'startDate');
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="text-lg font-semibold mb-4">월별 기타활동 건수 및 인정시간</h3>
      ...
      <div>
        <h3 className="text-lg font-semibold mb-4">연도별 기타활동 건수 및 인정시간</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart
            data={yearlyData}
            margin={{ top: 20, right: 30, left: 20, bottom: 5 }}
...
export default OtherActivityLogCharts;
